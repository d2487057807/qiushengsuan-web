/**
 * 注册页面
 * 按 UI 设计稿实现：图形验证码 + 手机/邮箱验证码 + 密码强度 + 确认密码
 */
...
export default function RegisterPage() {
  ...
  const formOk = accountOk && smsCode.length >= 6 &&
    ...
    !accountError && !pwError && !confirmError && agreed;
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10">
        ...
        <div className="w-full max-w-[440px] overflow-hidden"
          ...
          <div className="px-8 pt-8 pb-6 text-center">
            ...
            <div className="text-[#8B8FA3] text-sm mt-1.5">通过验证码注册账号</div>
          </div>

          {/* 表单 */}
          <div className="px-8 pb-7 flex flex-col gap-3.5">
            ...
            <div>
              <InputField icon="🔒" placeholder="设置密码（8-20位）" value={password}
            ...
            <InputField icon="🔒" placeholder="确认密码" value={confirmPw}
            ...
            >
              <div
                className="w-[16px] h-[16px] rounded flex items-center justify-center flex-shrink-0 mt-0.5 transition-colors"
                style={{
                  background: agreed ? '#00D68F' : 'transparent',
                  border: `1.5px solid ${agreed ? '#00D68F' : '#5A5D70'}`,
                }}
              >
                {agreed && <Check size={11} color="#0F1117" strokeWidth={3} />}
              </div>
              <span className="text-xs leading-[1.6]" style={{ color: '#8B8FA3' }}>
                我已阅读并同意
                <Link
                  to="/terms"
            ...
            {/* 提交按钮 */}
            <button onClick={handleRegister} disabled={!formOk || loading}
            ...
            <div className="text-center mt-1">
              <span className="text-[#8B8FA3] text-sm">已有账号？</span>
              <Link to="/login" className="text-[#4A9EFF] text-sm ml-1 hover:opacity-70 transition-opacity">去登录</Link>
            </div>
          </div>
        </div>
